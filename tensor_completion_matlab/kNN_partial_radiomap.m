function [error] = kNN_partial_radiomap(tensor, anchor_indicator, points_rss, positions)
% tensor is a partial radio map
% anchor_indicator: indicating the anchors that can be used for
% localization
...
k = 13; % num of nearest neighbor used.
...
for no_testing_point = 1:num_testing_points
    ...
    distance = sqrt(sum(tensor_rss_dif .^2, 3));
    % dealing with anchors
    distance = distance .* anchor_indicator + (ones(m,n) - anchor_indicator) * 100000;
    ...
    for iter = 1:length(index)
        ...
        %weight = 1 / (100 + norm([row - x, col - y]));
        weight = norm([row - x, col - y]);
        x_est = x_est + x;
        y_est = y_est + y;
    ...
    %norm([row - x_total/ weight_total, col - y_total/ weight_total]);
